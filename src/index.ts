export interface ListConstructor {
  new (): List<unknown>;
  new <V>(entires?: ReadonlyArray<readonly [V]> | null): List<V>;
  new <V>(iterable: Iterable<readonly [V]>): List<V>;
...
class List<V> extends Set<V> {
  ...
  public partition(
    ...
    const results: [List<V>, List<V>] = [
      new this.constructor[Symbol.species]<V>() as this,
      new this.constructor[Symbol.species]<V>() as this,
    ];
    for (const [val1, val2] of this.entries()) {
   ...
   * the lists may be different objects, but contain the same data.
   * @param {List} list List to compare with
   * @returns {boolean} Whether the lists have identical contents
   */
   ...
   * @param {List} other The other List to filter against
   * @returns {List}
   */
  public intersect(other: List<V>): List<V> {
   ...
   * @returns {List}
   */
  public difference(other: List<V>): List<V> {
    return other
